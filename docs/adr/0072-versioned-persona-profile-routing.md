...
- 状态：Proposed（等待 LIFE2.9 独立 Review）
- 日期：2026-08-01
- Schema：82（无迁移）
...
- 设置缺失或未知时 fail closed 选择 v2.2；施工期当前数据库明确写入 v2.2；
...
- v2.3 在 LIFE2.9 的认证列表为空，必须在 LIFE2.10 真实模型门通过后才能选择。
...
Observer 摘要允许显式从单一 profile Core 确定性派生，不能混合两个版本。生产 selector 仍是 v2.2，因此 LIFE2.9 不改变观察器人格；v2.3 发布时需同步验证选择后的摘要。
