...
- 状态：Accepted；CDS.10 独立 review 通过，2026-07-26 后续审计返工完成
- 日期：2026-07-25
...
2. Episode 提案只接受状态为 `pending`、策略为 `episode-group-v1` 且仍满足资格门的真实 `memory_episode_candidates`；输入绑定候选账本指纹和 2 到 20 个 Fragment 的完整来源。所选 Fragment 必须是候选顺序中的连续区间，不得添加候选外成员。低置信度、模型判断目标不一致或因果链不足时必须跳过；形成 Episode 时输出自身必须同时声明同一目标与完整因果链。
3. Saga 提案只接受状态为 `qualified`、策略为 `saga-group-v1`、未晋升且未耗尽重试的真实 `saga_group_candidates`；输入绑定候选账本指纹和 2 到 12 个 Episode 的完整来源。任何非 skip 提案至少选择两个 Episode，每个动作必须匹配候选模式和目标 Saga 状态；Episode/Saga 的 reason code 必须与动作、置信度和安全降级原因一一对应。
...
- 240 个规则安全回归场景由 12 组组成，覆盖两个 DecisionKind；另有 8 个带人工标签的原始叙事回归样本，经真实数据库候选生成路径评估。它不是独立 holdout，当前 accuracy 为 50.00%，macro precision/recall/F1 为 38.89%/50.00%/43.33%；该小样本只观察规则与标签在真实候选路径上的差异，不作为任何模式晋级依据。
- 低置信度选中、高影响 merge 自动执行、Shadow application、oracle 安全违规和 MEM 领域表写入均为 0。
- 独立 `cds10-narrative-safety-oracle-v3` 不读取 fixture expected，独立检查候选 provenance、来源与目标绑定、Episode 连续成员及语义动作矩阵、Saga 最小成员及原因矩阵、低置信度、恢复来源、merge 与应用安全不变量。
...
2026-07-26 对 CDS.4～10 做独立代码复核时发现，初版 validator 已约束成员、来源和高影响动作，但没有闭合 Episode 自述语义与 form/skip 的对应关系，也没有强制 Episode/Saga reason code 与动作一致。失败测试证明该缺口后，validator 与独立 oracle 同步收紧为 v3；规则固定集仍为 240/240，安全违规和 MEM 领域写入保持 0。
