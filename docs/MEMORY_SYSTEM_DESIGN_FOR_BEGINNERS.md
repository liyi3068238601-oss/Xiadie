...
状态：设计基线；阶段 A～E 与 F.1 已完成，阶段 F.2～F.8 待施工
...
| F 用户文件知识库 | F.1 已完成 | schema 28、知识/记忆/Lore 分域、配额、任务账本与删除边界已固定 |
...
- [ ] 前后端联合校验 `.txt`/`.md` 的扩展名、MIME、文件头、UTF-8 解码和真实大小。
- [ ] 在写数据库前执行单文件、文档数、总存储和解码字符配额；重复哈希返回明确结果。
- [ ] 只处理用户本次明确选择的文件，不扫描目录，不在数据库保存绝对原路径。
- [ ] 使用随机 storage key 原子复制到应用数据目录；失败不留下伪 document 或半成品。
- [ ] UI 在确认前展示类型、大小、本地/远程数据流向和敏感内容提醒。

#### F.3：可取消解析任务

- [ ] 实现 TXT/Markdown 本地解析、统一换行、标题结构提取和可诊断错误码。
- [ ] 实现串行任务认领、有限重试、陈旧恢复、协作取消和优雅停机。
- [ ] 任务事件、日志和 API 错误不包含原文、绝对路径或模型原始输出。
- [ ] 取消、进程中断、损坏编码和解析失败不会生成可检索产物。

#### F.4：稳定切片与来源定位

- [ ] 新增 chunk schema，保存稳定 ordinal、标题路径、段落范围、页码占位、字符范围和内容哈希。
- [ ] Markdown 按标题/段落边界切片，TXT 按段落边界切片；超长段落有确定性回退。
- [ ] 相同字节输入和相同协议版本产生完全相同的切片与定位。
- [ ] 后续加入 PDF/DOCX 时复用 locator 契约，不伪造页码。
...
- [ ] 每条结果提供文档 ID、文件名、标题路径、段落/页码定位和内容哈希短指纹。
- [ ] 回答引用可点击回到真实来源；找不到证据时不得生成虚构引用。
- [ ] 知识内容进入独立低权限资料区块，任何内嵌指令都不能改变人格、权限或工具策略。
- [ ] 召回预算按 token 控制，并与 Lore、相处记忆分别计量和审计。

#### F.7：管理界面、删除和重建

- [ ] 用真实文档列表、状态、进度和错误替换占位成功提示，并支持取消/重试。
- [ ] 提供搜索、collection/状态筛选、来源详情、重建索引和同名文件区分。
- [ ] 删除先退出召回，再清原文、切片、FTS、向量和派生产物；失败保持不可检索并可重试。
- [ ] 高风险清理二次确认，明确应用外备份不会同步删除。

#### F.8：格式扩展、可插拔向量与总验收

- [ ] 在独立沙箱边界加入 PDF、DOCX 解析，保留真实页码/段落和格式专属错误。
- [ ] 定义本地/远程 Embedding provider 接口；无向量时 FTS 完整可用，远程发送必须逐次披露同意。
- [ ] 向量索引可重建、可删除、可按 document 过滤，不成为删除残留旁路。
- [ ] 覆盖重复导入、同名不同内容、损坏文件、取消、删除失败、索引失败、提示注入和无向量降级。
- [ ] 完成导入→解析→切片→检索→引用→删除全链路及生产构建总验收后，才标记阶段 F 完成。
