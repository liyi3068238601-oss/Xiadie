...
| B 自主 Fragment 写入 | 未开始最终实现 | 当前仍是关键词候选与人工确认流程 |
...
- [ ] 写 ADR：自主写入事务、失败域、隐私边界和旧候选迁移方式。
- [ ] 新增 Fragment 的 scope、kind、importance、emotion、inner_reason、观察器版本和生命周期字段迁移。
- [ ] 新增观察恢复任务表、幂等键和有限重试字段。
- [ ] 新建 `memory_observer.py`，使用第 33 节模板和严格输出 schema。
- [ ] 把人格摘要、`derived.cluster`、最近对话和相关旧记忆传给观察器。
- [ ] 情绪状态不可用时使用 `cluster="neutral"`，记忆观察仍继续。
- [ ] 分别校验消息证据、摘要事实覆盖、枚举、分数、数量和敏感性。
- [ ] 通过校验后直接写正式 Fragment，不再进入逐条确认流程。
- [ ] 同一来源、同一事实的重复观察保持幂等，不重复写入。
- [ ] 密码、密钥、验证码和明确禁止记录的内容直接拒绝，正文不进入诊断日志。
- [ ] JSON 非法时执行受限修复；仍失败则创建 recovery_pending，不把抢救文本直接激活。
- [ ] 保留来源会话、来源消息、观察理由、观察器版本和完整审计事件。
- [ ] 将关键词规则降级为无模型时的保守兜底，不再作为主逻辑。
- [ ] 新增自主写入、恢复任务和观察调用的内部服务/API。
- [ ] 聊天界面显示受频率限制的轻量“已记住”提示。
- [ ] 记忆页展示自主写入原因、来源和观察器版本。
- [ ] 移除主界面的 Fragment 逐条确认入口，但保留编辑、纠错、禁用和删除。
- [ ] 补齐值得记、不值得记、敏感拒绝、注入攻击、重复调用和非法 JSON 测试。
- [ ] 模型失败、数据库失败和恢复任务失败均不影响聊天回复。
