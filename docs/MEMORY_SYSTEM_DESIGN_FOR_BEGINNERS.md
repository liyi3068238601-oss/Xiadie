...
状态：设计基线；阶段 A～C 与 D.1 已完成，阶段 D.2～F 待施工
...
| D Saga | D.1 已完成 | schema 18 与 ADR-0018 已建立正式数据地基；聚合服务和界面待施工 |
...
- [ ] 写纯本地 Saga 策略模块，固定窗口、阈值、权重、上限和策略版本。
- [ ] 只读取 active/completed 正式 Episode；候选、archived 和 tombstone 不参与默认预筛。
- [ ] 至少 2 个 Episode 且跨自然日；同日重复描述不能独立形成 Saga。
- [ ] 计算共同 Entity、文本主题、时间连续性和叙事连贯性四个可解释分量。
- [ ] 模型不能绕过共同主题最低门槛和本地综合阈值。
- [ ] 使用策略版本与有序 Episode ID 生成稳定分组指纹。
- [ ] 已属于有效 Saga 的 Episode 不再次进入候选；跨 Saga 冲突明确记录。
- [ ] 低分候选只留最小评分记录，不能修改 Episode 或创建正式 Saga。
- [ ] 增加精确阈值、跨日、无共同主题、重复运行和冲突测试。

#### 阶段 D.3：摘要生成与事实校验

- [ ] 写 Saga 摘要协议，模型只能使用来源 Episode 支持的 claim。
- [ ] 对标题、主题、当前阶段、关键变化和结束证据逐项校验。
- [ ] 保存模型、协议、证据 Episode、来源哈希、token 和修复审计。
- [ ] 非法 JSON 只允许一次受限修复；仍失败时使用安全抽取式摘要。
- [ ] 提示注入、无来源动机、虚构结果和错误时间不能进入正式摘要。
- [ ] 摘要期间来源变化时通过哈希复核拒绝旧结果。
- [ ] 增加正常、修复、回退、虚构、注入和 TOCTOU 测试。

#### 阶段 D.4：后台整理与原子应用

- [ ] 新增 Saga Consolidator run/event 账本、八态任务状态机和幂等 request key。
- [ ] worker 串行认领、有限重试、失败退避、陈旧任务恢复和协作式取消。
- [ ] 应用启动和超过六天的空闲周任务只入队，不阻塞聊天。
- [ ] 新建 Saga、来源关系、Entity、事件和 run 终态在单个短事务提交。
- [ ] 新 Episode 追加时先生成校验摘要，再复核旧来源哈希和 updated_at 后原子提交。
- [ ] 追加失败、模型失败或并发冲突不改变旧摘要、旧来源和旧状态。
- [ ] 重复运行、部分写入、取消、崩溃恢复和并发来源变化测试通过。
...
- [ ] 增加长期故事列表、详情、当前阶段和文字状态解释。
- [ ] 增加按时间排序的 Episode 时间线、来源状态和原对话入口。
- [ ] 展示摘要校验状态、短来源指纹、协议、实体和纠错记录。
- [ ] 提供 Saga 纠错与合法生命周期操作，不提供候选确认流程。
- [ ] 增加键盘操作、aria 属性、非颜色状态表达和窄屏布局。
- [ ] 复核旧 Episode 候选 API 的调用与迁移依赖，满足条件后写退役 ADR。
- [ ] 执行重复聚合、跨 Saga 冲突、增量失败、结束、恢复和摘要事实总验收。
- [ ] 更新基线、测试数量和阶段状态；阶段 D 所有小项通过后才标记完成。
