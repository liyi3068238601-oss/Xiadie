...
状态：设计基线；阶段 A 与阶段 B.1 已完成，B.2～F 待施工
...
| B 自主 Fragment 写入 | B.1 协议与数据地基已完成 | schema 10、严格协议和纯校验测试已提交；运行时仍是旧候选流程 |
...
- [ ] 选择跟随当前模型或独立记忆观察模型，并拒绝 mock 作为独立观察模型。
- [ ] 每轮成功对话只快速入队，模型调用不阻塞 SSE done。
- [ ] 保存模型、token、耗时、协议版本、状态与安全错误码，不保存失败敏感正文。
- [ ] 实现严格 JSON、围栏清理、完整对象提取和一次受限修复链。
- [ ] 成功结果只保存净化候选，本阶段仍不写正式 Fragment。
- [ ] 增加观察记录内部 API 和并发幂等、超时、非法 JSON、重启恢复测试。

#### 阶段 B.3：可靠后台执行与原子自主写入

- [ ] worker 串行处理 queued/recovery_pending，最多三次有限退避后 exhausted。
- [ ] 在单一事务内重新核对来源、去重、写 Fragment、实体关系、事件和 applied 状态。
- [ ] 同一来源、同一事实重复观察不重复写入，数据库失败整体回滚。
- [ ] 通过校验后直接写正式 Fragment，不再进入逐条确认流程。
- [ ] 保留来源会话、全部证据消息、观察理由、观察器版本和完整审计事件。
- [ ] 将关键词候选降级为无模型保守兜底，模型路径可用时不再作为主逻辑。
- [ ] 模型、数据库和恢复任务失败均不影响聊天回复。

#### 阶段 B.4：自主记忆界面切换

- [ ] 聊天界面显示受频率限制的轻量“已记住”提示。
- [ ] 记忆页展示自主写入原因、来源、scope、kind、importance 和观察器版本。
- [ ] 移除主界面的 Fragment 逐条确认入口，但保留旧候选的兼容管理入口。
- [ ] 保留编辑、纠错、禁用和删除；新增纠错专用 API 与审计语义。
- [ ] 完成旧候选迁移/退役策略后，才允许删除旧候选入口和相关代码。

### 阶段 C：Episode 自动化

- [ ] 写 ADR：Episode 自动合成事务、重复分组和失败恢复。
- [ ] 将现有候选器改为可排队、可取消、可审计的后台 Consolidator。
- [ ] 应用启动/空闲时检查任务，不使用阻塞聊天的定时循环。
- [ ] 保留 2～20 条、7 天窗口、共同实体和单一正式 Episode 归属边界。
- [ ] 实现 0.35/0.25/0.20/0.20 分组权重和 0.50 安全阈值。
- [ ] 低于阈值的分组保留 7 天重新评估，到期只清候选、不删 Fragment。
