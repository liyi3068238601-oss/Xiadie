...
- F.1 只建立数据地基，没有读取或复制真实文件；该边界已由下面的 F.2 安全准入实现取代，解析与索引
  仍保持未实现。
...
- 文件页已提供真实的导入前确认、仅本地数据流向和敏感标记，并展示 queued 文档；解析、切片、索引和
  对话引用仍未实现，界面明确显示“等待解析”。当前后端 256 项、前端 18 项及生产/Electron 检查通过。
