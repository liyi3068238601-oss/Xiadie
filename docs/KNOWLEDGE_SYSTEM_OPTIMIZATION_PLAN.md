...
- [ ] K.9 尚未验收：现有 402 项后端测试、33 项前端测试和生产构建通过，但仍需执行 K.9 的模式、Provider、授权、安全、打包和冻结后端总矩阵。
...
恢复后的下一步：从 K.9 开始执行知识系统总验收；完成后按本计划审查规则关闭阶段。
...
- 查询：自然召回实际使用确定性清理结果；英文术语、数字和名称保留。代词延续只附加最近两轮中出现且能由本地已索引文档名称/标签验证的实体，最多两项；显式检索不改写。
