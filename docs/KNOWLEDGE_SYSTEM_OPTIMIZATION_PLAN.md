...
- [ ] 重新运行当前全量测试并记录真实数量、schema 和最新提交。
- [ ] 建立自然召回、远传策略和一次性授权 ADR。
- [ ] 固定默认仍为 explicit，不改变旧用户行为。
- [ ] 固定“本地检索不等于远传许可”的安全边界。
- [ ] 建立匿名固定评测资料和问题分类骨架。
- [ ] 记录本阶段 review 采纳、调整、推迟和拒绝项。
...
- [ ] 建立版本化 RecallDecision 协议和稳定 reason code。
- [ ] 实现显式允许、显式禁止、寒暄/情绪/简单任务跳过规则。
- [ ] 使用本地 FTS+dense 生成候选统计，不调用远程分类模型。
- [ ] 新增 shadow 模式；只记录无正文决策，不改变模型上下文。
- [ ] 设置超时和异常降级，不能拖慢聊天完成。
- [ ] 增加开发诊断读取 API 和最小 UI。
- [ ] 覆盖模型未加载、FTS 无词项、向量失败和空库。
