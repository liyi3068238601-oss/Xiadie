...
状态：设计基线，阶段一内核、阶段 1.1 加固、阶段 2.1 协议层与阶段 2.2 调用审计已完成
...
- [x] 输入超过 20000 字符或输出超过 12000 字符时保守跳过/拒绝。
...
- [ ] 根据 `next_attempt_at` 实际执行最多 3 次有限重试和退避。
- [ ] 为观察器增加独立轻量模型选择；当前暂时复用本轮聊天供应商与模型。
- [ ] 将候选 delta 与最新状态合并并原子应用。
- [ ] 观察调用改为不延迟聊天 done 的可靠后台任务。

验收：真实模型可以生成可审计候选；任何观察失败都不破坏回复、不泄露原始失败文本，也不会提前改变状态。

### 阶段 3：遐蝶语调网格

- [ ] 完成 9 情绪簇 × 5 guardedness 档的角色文案。
- [ ] 实现缺项回退与 contact_need 附加层。
- [ ] 用固定对话集检查人格连续性。
- [ ] 防止低落、烦躁状态产生攻击、操纵或索取安慰。
