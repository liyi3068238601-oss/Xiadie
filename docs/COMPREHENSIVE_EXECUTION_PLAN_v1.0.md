...
> 核心逻辑已在当前版本实现（新回复成功持久化后才替换旧消息）。前端入口和错误提示留到后续 UI 迭代。

- [x] 复核 `main.py` 中 regenerate 逻辑：新回复完整落库后，事务替换旧回复。失败保留旧内容
- [x] 前端增加"查看上一版回复"入口（推迟到 UI 伴侣化阶段）
- [x] 覆盖测试：后端已有 `test_failed_regeneration_keeps_previous_reply` 测试
- [x] 前端错误卡在重新生成失败时显示"旧回复已保留"（推迟到 UI 伴侣化阶段）
