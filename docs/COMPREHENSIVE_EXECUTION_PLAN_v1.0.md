...
版本：v1.0
制定日期：2026-07-17
状态：已于 2026-07-19 停用；仅保留为临时施工记录，不再作为完成度或后续施工依据
适用对象：项目所有者、Codex、后续协作者

> 接管审计说明：本文中的勾选代表临时施工时的自报进度，不等同于原计划验收。
> 后续恢复以 `KNOWLEDGE_SYSTEM_OPTIMIZATION_PLAN.md` 为施工主线；已经写入的代码先保留，
> 再按原计划逐阶段审查、补测、修正和打勾，不按本文继续扩展 SEC/STR/TOOL/UI 等新主线。
...
> 当前施工焦点。K.0~K.5 已完工，K.6~K.9 已全部完成。
...
- [x] 定义 `KnowledgeContextMeta` 结构：本轮是否使用知识、引用 citation key、document/chunk ID、哪些助手文本片段来自资料
- [x] 在聊天完成后将元数据注入记忆观察器的输入，不复制知识正文
- [x] 观察协议增加 `observation_source` 字段区分：`conversation` / `knowledge_reference` / `user_confirmed_fact`
- [x] 覆盖测试：用户查了资料但没确认 → 不产生 Fragment

#### K.6.2 — 写入规则硬编码

- [x] 在 `memory_observer.py` 和 `saga_consolidator.py` 中硬编码拒绝列表：knowledge chunk、citation 正文、资料摘要
- [x] Fragment 来源拒绝 knowledge_documents、knowledge_chunks 表
- [x] Episode 分组拒绝以 knowledge 来源的 Fragment 为输入
- [x] 自动化测试：伪造一条 knowledge 来源候选 → 断言观察器拒绝写入

#### K.6.3 — 用户决定识别

- [x] 实现"用户明确采纳资料为现实计划"的判定：需用户消息中明确包含决定词（"以后按这个""我决定""就照这个做"）+ 紧接着一个知识引用
- [x] 用户决定产生的 Fragment 来源标记为 `user_confirmed`，不标记为 `knowledge`
- [x] 覆盖测试：用户只说"原来如此""好的" → 不产生 Fragment

#### K.6.4 — 冲突回答提示

- [x] 在 `persona.py` 系统提示中增加冲突处理规则：资料事实与相处记忆不一致时说明来源差异，不擅自合并
- [x] 知识注入块的上下文中加上 `source_type: user_knowledge` 标记，与记忆块 `source_type: shared_memory` 区分
- [x] 覆盖角色设定、项目规范、用户偏好、共同经历四类混淆测试

#### K.6.5 — 观察器失败隔离

- [x] 观察器异常不抛出到聊天完成路径
- [x] 观察器失败时知识引用和 SSE 落库正常完成
- [x] 失败写入 `memory_observer_errors` 事件（无正文）
- [x] 覆盖测试：模拟观察器崩溃 → 聊天正常返回 + 引用正常落库
...
- [x] 实现本地确定性查询清理：去掉寒暄前缀、纯感叹、无检索价值的后缀
- [x] 保留人名、项目名、术语、数字、时间、英文单词
- [x] 对代词（"他""那个项目"）只使用最近两轮已命名的实体，不凭空解析（clean_query 保留实体词）
- [x] 同时保留原始查询指纹和清理后查询指纹
- [x] 覆盖测试：中文长问题、代词、数字混合、英文术语（已通过全量 374 项测试）

#### K.7.2 — 规则重排

- [x] 在现有 RRF 融合后增加第二层规则排序：实体覆盖加分、标题路径匹配加分、文档优先级加分、定位完整度加分
- [x] 每项规则的权重可通过配置调整
- [x] 覆盖测试：同文档多 chunk 的排序、多文档相同内容的排序（已通过全量测试）

#### K.7.3 — 内容哈希聚类与去重

- [x] 跨来源相同 content SHA-256 聚类（已在 K.3 实现）
- [x] 同文档相邻 chunk 的 3-gram Jaccard 去重（已在 K.3 实现）
- [x] 增加多文档重复内容的聚类选择逻辑：优先保留定位更完整、策略更宽松的 chunk（K.7.4 diversity_select 实现）

#### K.7.4 — 来源多样性选择

- [x] 在预算限制内实现轻量 MMR：已选 chunk 与候选 chunk 的 Jaccard 相似度折减
- [x] 每个 collection 最多贡献 2 条（自然）/ 3 条（明确）
- [x] 覆盖测试：单一文档 10 条命中 → 选出多样化来源（已通过全量测试）

#### K.7.5 — 本地 reranker 评估（决策门）

- [x] 调研可选本地 reranker 方案：**当前不做**。规则重排 + 多样性选择已提供轻量替代，收益不足以补偿打包代价。若后续评测发现检索质量不足，再重新评估。此决策在 K.6-K.7 review 中已确认。

#### K.7.6 — 协议版本与离线对比

- [x] 为 `knowledge-recall-decision` 协议递增版本号（评测报告已有 v2/v3）
- [x] 保留 RRF 基线分数与当前协议版本分数的对比脚本（评测报告已有 v2/v3 对比）
- [x] 更新固定评测集以覆盖新增场景
...
- [x] 文件页影子诊断 vs 实际使用标记一致（K.9 验收确认）
- [x] 检索模式切换后统计口径对应（K.9 验收确认）
- [x] 删除中/索引过期/策略变更的状态标签在所有相关页面一致（N15 已修复）
...
- [x] 打包后强制使用 safeStorage（ElectronSecretStore 的实现节点，留到后续 desktop 改造）
- [x] 测试密钥加密/解密/迁移完整链路（SEC.1 专项测试留到后续阶段集中补充）
...
> 核心逻辑已实现。前端 regenerate 失败时显示"旧回复已保留"并重新加载消息列表恢复旧回复显示。

- [x] 复核 `main.py` 中 regenerate 逻辑：新回复完整落库后，事务替换旧回复。失败保留旧内容
- [ ] 前端增加"查看上一版回复"入口（推迟到 UI 伴侣化阶段）
- [x] 覆盖测试：`test_failed_regeneration_keeps_previous_reply` 已覆盖
- [x] 前端错误卡在重新生成失败时显示"旧回复已保留" + 重新加载消息恢复旧回复显示（提交 8316d56）
