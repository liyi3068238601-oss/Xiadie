...
EAP PR #1 已于 2026-07-22 合并，CDS ConstructionBaseline 固定为 `main@6b8aa47134f8a9a55131c73bb1148e6912421c4f`、Schema 60、冻结协议和后端 `937 passed, 1 warning`。CDS.0～CDS.10 已施工并分别通过 strict review，Schema 61/62 建立统一决策账本与模型运行时，之后未新增迁移。当前 9 个 DecisionKind 全部最高为 Shadow，EAP、CTX、Knowledge 与 MEM 的 application ownership 均未转移。2026-07-26 review 后代码审计发现并修复 CDS.10 Episode/Saga 语义动作矩阵缺口与 CDS.6 SSE final 重复回调；全量后端当时为 `2284 passed, 1 warning`，前端 `45 passed`。CDS.10 的 8 条未独立评审叙事样本 accuracy 仅 50%，只作观察且明确不具备晋级证据资格。CDS.11 已冻结 `specialty-adapter-contract-v1` 与只读 `eap-decision-run-adapter-v1`：LIFE 仅提供 revision/hash 来源，KIG 仅提供已校验候选，EAP 保留全部主动候选与投递写权；未实现 LIFE/KIG 领域能力，未新增迁移；当前后端全量为 `2290 passed, 1 warning`。当前停在 CDS.11，等待独立 review；下一可用 Schema 仍为 63。
