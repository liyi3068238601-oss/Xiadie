...
- 当前专项：`docs/EMOTION_RELATIONSHIP_AND_PROACTIVE_COMPANION_PLAN.md` 第 9.B 节 EAP.R0～EAP.R6。Schema 48～55 和领域模块已落地，
  但真实候选、决策、表达、投递和反馈闭环未完成；不得另建重复情绪状态源，也不得提前宣称协议冻结。
...
该专项只执行第 9.B 节 EAP.R0～EAP.R6；EAP.A～EAP.J 是历史计划，不再代表当前完成度。Schema 48～55 不回写，收口从 Schema 56 开始；当前已完成 R0～R4，Schema 到 59，下一阶段为 R5。R4 已接入默认关闭、显式实验开启的本机 Delivery 账本和 Level 0～4 通道；所有真实输出仍受后端最终授权复核与 at-most-once 状态机约束。
R5 完成反馈、历史和用户纠正闭环前不得正式发布主动投递；Level 5 外部渠道保持硬禁用。LIFE 在 EAP.R6 正式冻结后使用下一个可用 schema 版本。
