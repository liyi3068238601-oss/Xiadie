...
> 2026-08-01 后续策略变更：产品现改为默认允许摘要模型处理生成摘要所需的历史对话，旧授权位不再
> 阻断任务；Provider 位置与 revision 绑定仍保留。设置页同步公开解析后的模型和数据位置。本段保留的是
> 2026-07-19 Review 当时的验证事实。
...
- 远程历史处理在 2026-07-19 Review 时仅有后端配置/API，未增加普通陪伴界面的技术开关。
  2026-08-01 策略改为默认放行且不提供阻断开关；高级设置改为透明展示摘要模型、执行位置和历史正文远传说明。
