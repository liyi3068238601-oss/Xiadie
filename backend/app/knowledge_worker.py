...
from . import db, knowledge, knowledge_parser
...
async def _process_claimed(row: dict) -> None:
    ...
    try:
        ...
        _mark_failure(row, "knowledge_parse_failed")
    except Exception:  # noqa: BLE001
        _logger.exception("Knowledge parse run failed")
        _mark_failure(row, "knowledge_parse_failed")
...
def _claim_next() -> dict | None:
    ...
    try:
        ...
        row = conn.execute(
            ...
            " AND r.current_stage IN ('validation','copy','parsing')"
        ...
        cursor = conn.execute(
            ...
            (PARSING_PROGRESS, attempt, now, now, row["id"], row["status"]),
        )
        ...
        _event(conn, row["id"], "parsing_started", row["status"], "running", "parsing", None,
               {"attempt": attempt}, now)
        conn.commit()
        result = dict(row)
        result.update(status="running", current_stage="parsing", attempt_count=attempt)
        return result
    finally:
        conn.close()
    ...
    try:
        ...
        try:
            ...
            conn.execute(
                ...
                "progress=?,error_code=NULL,updated_at=? WHERE id=? AND status='running'",
    ...
    try:
        ...
        for row in rows:
            ...
            _event(conn, row["id"], "failed" if exhausted else "recovery_scheduled",
                   "running", after, "parsing", error, {}, at)
        conn.commit()
    finally:
        conn.close()
    for key in cleanup:
...
def _mark_failure(row: dict, code: str) -> None:
    ...
    try:
        ...
        _event(conn, row["id"], "failed" if exhausted else "retry_scheduled", "running",
               after, "parsing", code, {"attempt": current["attempt_count"]}, now)
        conn.commit()
    finally:
        conn.close()
